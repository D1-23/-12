...
export const ROW_OVERHEAD_PX = 8;
...
export function estimateUnitHeight(
  ...
  if (level === 'block') return 120 + ROW_OVERHEAD_PX;

  const actualLineHeight = Math.round(fontSize * 1.4);
  const baseHeight = UNIT_MIN_HEIGHT;
  const charWidth = Math.round(fontSize * 0.85);
  const tdPadding = 12;

  const valueEffectiveWidth = Math.max(1, valueWidthPx - tdPadding);
  const valueCharPerLine = Math.max(1, Math.floor(valueEffectiveWidth / charWidth));
  let valueLines = 1;
  if (formattedValue.length > 0) {
    const explicitLines = formattedValue.split('\n').length;
    const wrappedLines = Math.ceil(formattedValue.length / valueCharPerLine);
    valueLines = Math.max(explicitLines, wrappedLines);
  }

  const labelEffectiveWidth = Math.max(1, labelWidthPx - tdPadding);
  const labelCharPerLine = Math.max(1, Math.floor(labelEffectiveWidth / charWidth));
  let labelLines = 1;
  if (label.length > 0) {
    labelLines = Math.ceil(label.length / labelCharPerLine);
  }

  const lineCount = Math.max(valueLines, labelLines);
  return Math.max(baseHeight, lineCount * actualLineHeight + 8);
}
